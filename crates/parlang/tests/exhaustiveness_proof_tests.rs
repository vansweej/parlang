...
fn prove_exhaustive_option_match() {
    ...
    assert!(
        result.is_ok(),
        "Exhaustive match should succeed: {:?}",
        result.err()
    );
...
fn prove_non_exhaustive_option_missing_some() {
    ...
    // Patterns: only None
    let patterns = vec![Pattern::Constructor("None".to_string(), vec![])];
...
fn prove_variable_makes_exhaustive() {
    ...
    assert!(
        ...
        "Variable pattern should make it exhaustive"
    );
}
...
fn prove_int_patterns_non_exhaustive() {
    ...
    assert!(
        !result.is_exhaustive(),
        "Integer patterns without wildcard should be non-exhaustive"
    );
}
...
fn prove_int_patterns_with_wildcard_exhaustive() {
    ...
    assert!(
        result.is_exhaustive(),
        "Wildcard should make integer patterns exhaustive"
    );
}
...
fn prove_empty_patterns_non_exhaustive() {
    ...
    assert!(
        !result.is_exhaustive(),
        "Empty pattern list should be non-exhaustive"
    );
}
...
fn prove_multiple_constructors_all_covered() {
    ...
    assert!(
        result.is_exhaustive(),
        "All constructors covered should be exhaustive"
    );
}
...
fn prove_multiple_constructors_some_missing() {
    ...
    if let parlang::ExhaustivenessResult::NonExhaustive(missing) = result {
        ...
        assert!(
            missing.contains(&"Archived".to_string()),
